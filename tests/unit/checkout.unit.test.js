...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      const initialWalletBalance = userOneFinal.walletMoney
      console.log("Test Bal:" +initialWalletBalance)
      ...
      let total = 0; 
      cartWithProductsUserOne.cartItems.forEach(i => {
          total += (i.product.cost * i.quantity)
      })
      const finalBalance = initialWalletBalance - total
      console.log("Final Bal" + finalBalance )

      expect(finalBalance).toBeLessThan(initialWalletBalance)
      //  expect(true).toEqual(false);
    });
  });
});
