...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if(!cart){
    ...
    if(!productInDB) { throw new ApiError(httpStatus.BAD_REQUEST, "Product doesn't exist in database")}
    const productInCart = await cart.cartItems.filter(obj => obj.product._id == productId)
    if(!productInCart.length){
      throw new ApiError(httpStatus.BAD_REQUEST, "Product not in Cart")
    }

    const index = await cart.cartItems.findIndex(obj => obj.product._id == productId)
